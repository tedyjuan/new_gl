...
class C_customers_budget extends CI_Controller
{
	function __construct()
	{
		parent::__construct();
		is_logged_in();
		$this->load->model('M_customers_budget');
		$this->load->model('M_global');
	}
	function index()
	{
		$data['judul']      = 'List Budget Offer';
		$data['load_grid']  = 'C_customers_budget';
		$data['load_add']   = 'C_customers_budget/add';
		$data['url_delete'] = 'C_customers_budget/delete';
		$this->load->view("v_customers_budget/grid_customers_budget", $data);
	}
	public function griddata()
	{
		...
		$data = $this->M_customers_budget->get_paginated_ord_customer_budget($length, $start, $search, $order_by, $dir);
		$total_records = $this->M_customers_budget->count_all_ord_customer_budget();
		$total_filtered = $this->M_customers_budget->count_filtered_ord_customer_budget($search);

		// URL helper
		$url_edit   = 'C_customers_budget/editform/';
		$url_delete = 'C_customers_budget/hapusdata/';
		$url_status = 'C_customers_budget/editform_status/';
		$url_detail = 'C_customers_budget/editform_detail/';
		$load_grid  = 'C_customers_budget/griddata';
	...
	function add()
	{
		...
		$data['load_refresh'] = 'C_customers_budget/add';
		$data['load_back'] = 'C_customers_budget';
		$this->load->view("v_customers_budget/add_customers_budget", $data);
	}
	public function simpandata()
	{
		...
		$html = $this->load->view('v_customers_budget/pdf_customers_budget', [
	...
	public function editform($uuid)
	{
		...
		// Load view edit
		$this->load->view('v_customers_budget/edit_customers_budget', [
			'judul'        => 'Edit Budget Offer',
			'load_back'    => 'C_customers_budget',
			'load_refresh' => 'C_customers_budget/editform/' . $uuid,
			'data'         => $dataHeader,
			'items'        => $dataItems,
			'uuid'         => $uuid
		]);
	...
	public function editform_detail($uuid)
	{
		...
		// Load view edit
		$this->load->view('v_customers_budget/detail_customers_budget', [
			'judul'        => 'Edit Budget Offer',
			'load_back'    => 'C_customers_budget',
			'load_refresh' => 'C_customers_budget/editform_detail/' . $uuid,
			'data'         => $dataHeader,
			'items'        => $dataItems,
			'uuid'         => $uuid
		]);
	...
	public function editform_status($uuid)
	{
		...
		// Load view edit
		$this->load->view('v_customers_budget/edit_status_customers_budget', [
			'judul'        => 'Edit Status Budget Offer',
			'load_back'    => 'C_customers_budget',
			'load_refresh' => 'C_customers_budget/editform_status/' . $uuid,
			'data'         => $dataHeader,
			'uuid'         => $uuid
		]);
	}
	...
	{
		$get_value = $this->input->get('getcustomers_budget');
